...
from clients.exercises.exercises_schema import CreateExerciseRequestShema, CreateExerciseResponseShema, \
    GetExerciseResponseShema, ExerciseSchema, UpdateExerciseResponseShema, UpdateExerciseRequestSchema, \
...
def assert_create_exercise_response(request: CreateExerciseRequestShema, response: CreateExerciseResponseShema):
    """
    ...
    :raises AssertionError: Если хотя бы одно поле не совпадает.
    """
    logger.info('Checking create exercise response')
...
def assert_get_exercise_response(
        get_exercise_response: GetExerciseResponseShema,
        create_exercise_response: CreateExerciseResponseShema
):
    """
    Проверяет, что ответ на получения задания соответствует запросу.
    :param get_exercise_response: Фактические данные задания
    :param create_exercise_response: Ожидаемые данные задания
    :raises AssertionError: Если хотя бы одно поле не совпадает.
    """
    logger.info('Checking get exercise response')
    assert_exercise(get_exercise_response.exercise, create_exercise_response.exercise)


@allure.step('Check update exercise response')
def assert_update_exercise_response(
        response: UpdateExerciseResponseShema,
    ...
    """
        ...
        :param request: Данные которые были направлены для обновления задания
        :raises AssertionError: Если хотя бы одно поле не совпадает.
        """
    logger.info('Checking update exercise response')
    assert_equal(response.exercise.title, request.title, 'title')
    assert_equal(response.exercise.max_score, request.max_score, 'max_score')
    assert_equal(response.exercise.min_score, request.min_score, 'min_score')
    assert_equal(response.exercise.order_index, request.order_index, 'order_index')
...
def assert_exercise_not_found_response(actual: InternalErrorResponseSchema):
    ...
    logger.info('Checking exercise not found response')
    assert_internal_error_response(actual, expected)


@allure.step('Check get all exercises response')
def assert_get_exercises_response(
        get_exercises_response: GetExercisesResponseSchema,
        create_exercise_responses: list[CreateExerciseResponseShema]
):
    """
    ...
    """
    logger.info('Checking get all exercises response')
